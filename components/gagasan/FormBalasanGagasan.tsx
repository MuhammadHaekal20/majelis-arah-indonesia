...
import {
  createBalasanGagasan,
  type BalasanActionState,
} from "@/lib/actions/gagasan";

const initialState: BalasanActionState = { ok: false, message: "" };

type FormBalasanGagasanProps = {
  gagasanId: string;
};

export function FormBalasanGagasan({ gagasanId }: FormBalasanGagasanProps) {
  ...
  return (
    <form ref={formRef} action={formAction} className="space-y-3">
      <input type="hidden" name="gagasan_id" value={gagasanId} />
      <label className="block space-y-1.5">
        <span className="text-sm font-medium text-mai-dark">
          Tanggapi sebagai {session.user.name}
        </span>
        <textarea
          name="konten"
          required
      ...
      >
        {pending ? "Mengirim…" : "Kirim tanggapan"}
      </button>
    </form>
  );
}
