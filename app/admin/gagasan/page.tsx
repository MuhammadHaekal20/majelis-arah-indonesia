...
import { moderateGagasan } from "@/lib/actions/gagasan";
...
export default async function AdminGagasanPage() {
  const items = await prisma.gagasanThread.findMany({
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-500">
          Tinjau pengajuan gagasan yang menunggu persetujuan.
        </p>
      </div>
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  ...
                  >
                    <td className="max-w-[12rem] px-4 py-3 font-medium text-mai-dark">
                      {item.judul}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-2">
                        <form
                          action={moderateGagasan.bind(null, item.id, "APPROVED")}
                        >
                          <button
                            type="submit"
                        ...
                        <form
                          action={moderateGagasan.bind(null, item.id, "REJECTED")}
                        >
                          <button
                            type="submit"
